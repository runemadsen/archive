...
test("extracts dimensions + orientation from a PNG", async () => {
  ...
  const { metadata } = await p.extract({
    loadBuffer: async () => png(1920, 1080),
    mimeType: "image/png",
    filename: "w.png",
  });
...
test("thumbnail.generate returns null for an undecodable buffer", async () => {
  const { thumbnail } = imagePlugin();
  assert.equal(
    await thumbnail.generate(src(Buffer.from("nope"), "x.png")),
    null,
  );
});
...
test("preview: web image points at the download; RAW uses the thumbnail", () => {
  const p = imagePlugin();
  assert.match(
    p.preview({ id: 9, original_filename: "a.jpg" }, helpers()),
    /src="\/api\/files\/9\/download"/,
  );
  const raw = p.preview(
    { id: 9, original_filename: "x.arw", thumbnail_type: "image/webp" },
    helpers(),
  );
  assert.match(raw, /src="\/api\/files\/9\/thumbnail"/);
  assert.equal(
    p.preview(
      { id: 9, original_filename: "x.arw", thumbnail_type: null },
      helpers(),
    ),
    null,
  );
...
test("publicEmbed: plain-original embed + srcset snippet", () => {
  const p = imagePlugin();
  const html = p.publicEmbed(
    { id: 9, original_filename: "a.jpg" },
    helpers(true),
  );
...
test("parseSpecParams + normalizeSpec clamp and validate a spec segment", () => {
  ...
  assert.deepEqual(
    normalizeSpec({ w: "800", h: "600", q: "80", fit: "cover" }),
    {
      width: 800,
      height: 600,
      fit: "cover",
...
test("serving lists image output formats", () => {
  assert.deepEqual(imagePlugin().serving.formats, [
    "webp",
    "jpg",
    "jpeg",
    "png",
...
test("serving.serve resizes + reformats a real image", async () => {
  ...
  const out = await serve(
    p,
    src(await realImage(1200, 800), "big.png"),
    "w=256.webp",
  );
...
test("serving.serve reformats without resizing (jpg -> jpeg encoder)", async () => {
  ...
  const out = await serve(
    p,
    src(await realImage(64, 48), "x.png"),
    "photo.jpg",
  );
...
test("serving.serve returns null for an undecodable buffer", async () => {
  const out = await serve(
    imagePlugin(),
    src(Buffer.from("not an image"), "x.png"),
    "w=100.webp",
  );
...
test("serving.serve throws on an invalid transform param (→ core 400)", async () => {
  ...
  await assert.rejects(
    () => serve(imagePlugin(), s, "fit=nonsense.webp"),
    /fit/,
  );
});
...
test("serving.serve renders RAW from its embedded JPEG preview", async () => {
  ...
  const out = await serve(
    imagePlugin(),
    src(raf, "DSCF1.RAF", "application/octet-stream"),
    "w=200.webp",
  );
