...
test("selectLadder never upscales; caps to source height", () => {
  ...
  assert.deepEqual(
    selectLadder(1080, DEFAULT_LADDER).map((r) => r.height),
    [1080, 720, 360],
  );
  // 720 source → drop 1080.
  assert.deepEqual(
    selectLadder(720, DEFAULT_LADDER).map((r) => r.height),
    [720, 360],
  );
  // 500 source → only 360 fits.
  assert.deepEqual(
    selectLadder(500, DEFAULT_LADDER).map((r) => r.height),
    [360],
  );
...
test("serving.serve resolves a bundle member via api.member with the right content-type", async () => {
  ...
  const api = {
    member: (memberPath, contentType) => ({ memberPath, contentType }),
  };
  const master = await serving.serve(
    { segments: ["master.m3u8"], ext: "m3u8" },
    api,
  );
  assert.deepEqual(master, {
    memberPath: "master.m3u8",
    contentType: "application/vnd.apple.mpegurl",
  });
  const seg = await serving.serve(
    { segments: ["360p", "seg_000.ts"], ext: "ts" },
    api,
  );
  assert.deepEqual(seg, {
    memberPath: "360p/seg_000.ts",
...
test("preview: HLS player when streamable, progressive fallback otherwise", () => {
  ...
  const streamable = p.preview(
    { id: 1, stream_type: "hls", thumbnail_type: "image/webp" },
    h,
  );
...
test(
  "extract + thumbnail + HLS build from a generated clip",
  ...
  async () => {
    ...
    try {
      ...
      const source = {
        ...
        filename: "clip.mp4",
        mimeType: "video/mp4",
        loadBuffer: async () => fsp.readFile(clip),
      };
      ...
      assert.ok(
        Buffer.isBuffer(thumb) && thumb.length > 0,
        "poster frame produced",
      );
      ...
      await buildHls(
        clip,
        out,
        selectLadder(meta.height, DEFAULT_LADDER),
        meta.hasAudio,
      );
...
async function buildClip(dest) {
  ...
  await new Promise((resolve, reject) => {
    ...
    child.on("close", (c) =>
      c === 0 ? resolve() : reject(new Error(`ffmpeg clip exit ${c}`)),
    );
  });
