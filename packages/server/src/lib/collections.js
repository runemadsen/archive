...
export function getCollection(db, id) {
  return (
    db
      .prepare(
        "SELECT id, name, parent_id, visibility, created_at, updated_at FROM collections WHERE id = ?",
      )
      .get(id) ?? null
  );
}
...
export function createCollection(db, { name, parentId = null, userId = null }) {
  ...
  try {
    const id = db
      .prepare(
        "INSERT INTO collections (name, parent_id, created_by) VALUES (?, ?, ?)",
      )
...
export function updateCollection(db, id, { name, parentId, visibility } = {}) {
  ...
  try {
    if (name !== undefined) {
      ...
      db.prepare(
        "UPDATE collections SET name = ?, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id = ?",
      ).run(trimmed, id);
    }
    ...
    if (visibility !== undefined) {
      ...
      db.prepare(
        "UPDATE collections SET visibility = ?, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id = ?",
      ).run(visibility, id);
    }
...
function moveSubtree(db, id, newParentId) {
  if (newParentId != null) {
    if (newParentId === id)
      throw new HttpError(400, "A collection cannot be its own parent");
    if (!getCollection(db, newParentId))
      throw new HttpError(404, "Parent collection not found");
    ...
    const inside = db
      ...
      .get(id, newParentId);
    if (inside)
      throw new HttpError(400, "Cannot move a collection into its own subtree");
  }
...
export function addFileToCollection(db, fileId, collectionId) {
  if (!getCollection(db, collectionId))
    throw new HttpError(404, "Collection not found");
  if (
    !db
      .prepare("SELECT 1 FROM files WHERE id = ? AND deleted_at IS NULL")
      .get(fileId)
  )
...
export function removeFilesFromCollection(db, collectionId, fileIds) {
  ...
  try {
    for (const fileId of fileIds)
      removeFileFromCollection(db, fileId, collectionId);
    db.exec("COMMIT");
  } catch (err) {
    db.exec("ROLLBACK");
    throw err;
