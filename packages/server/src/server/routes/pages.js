...
export function registerPageRoutes(router) {
  ...
  router.get("/", (req, res, ctx) => {
    ...
    let result;
    try {
      result = paginatedSearch(ctx.db, {
        ...state,
        query: composeQuery(state.text, state.filters),
      });
    } catch (err) {
  ...
  router.get("/files/:id", (req, res, ctx) => {
    ...
    const matches =
      ctx.registry?.matching?.(file.mime_type, file.original_filename) || [];
    const previewPlugin = matches.find((p) => p.preview);
    const preview = previewPlugin
      ? previewPlugin.preview(
          file,
          previewHelpers(previewPlugin, file, { isPublic }),
        ) || ""
      : "";
    const embedPlugin = isPublic ? matches.find((p) => p.publicEmbed) : null;
    const publicEmbed = embedPlugin
      ? embedPlugin.publicEmbed(
          file,
          previewHelpers(embedPlugin, file, { isPublic }),
        ) || ""
      : "";
    sendHtml(
      res,
