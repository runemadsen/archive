...
export async function runExtraction(db, ctx, fileId) {
  ...
  const { thumbnailType, streamType } = await pregenerateArtifacts(
    ctx,
    file,
    mimeType,
    filename,
  ...
  return {
    ok: true,
    thumbnail: Boolean(thumbnailType),
    stream: Boolean(streamType),
    pluginErrors,
  };
}
 ...
 */
async function pregenerateArtifacts(
  ctx,
  file,
  mimeType,
  filename,
  ...
  if (plugin) {
    try {
      ...
      pluginErrors.push({
        plugin: plugin.id,
        message: `pregenerate failed: ${err.message}`,
      });
    }
  }
