...
function setFileStatus(db, fileId, status) {
  db.prepare("UPDATE files SET extraction_status = ? WHERE id = ?").run(
    status,
    fileId,
  );
}
...
export class ExtractionWorker {
  constructor(
    db,
    { dataDir, registry, intervalMs = 250, renditions, events } = {},
  ) {
    if (!registry)
      throw new Error("ExtractionWorker requires a plugin registry");
    this.db = db;
    this.ctx = {
