...
const names = (db, q) =>
  searchFiles(db, q)
    .items.map((i) => i.original_filename)
    .sort();

test("collection is recognized as a filter key in the query string", () => {
  assert.deepEqual(parseQueryString("collection=Trips"), {
    text: "",
    filters: { collection: ["Trips"] },
  });
  assert.deepEqual(
    parseQueryString("mountains collection=Trips,Photos").filters,
    {
...
test("multi-select ORs collection names; combines (AND) with other filters", () => {
  ...
  assert.deepEqual(names(db, "collection=Trips,Docs"), [
    "note.txt",
    "photo.jpg",
  ]);
  // AND with a facet: only the image in Trips
  assert.deepEqual(names(db, "collection=Trips,Docs type=image"), [
    "photo.jpg",
  ]);
  db.close();
});
