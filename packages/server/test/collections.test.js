...
const descendants = (db, id) =>
  db
    .prepare(
      "SELECT descendant FROM collection_closure WHERE ancestor = ? ORDER BY descendant",
    )
    .all(id)
    .map((r) => r.descendant);
...
test("closure: create builds self + ancestor rows", () => {
  ...
  assert.equal(
    db
      .prepare(
        "SELECT depth FROM collection_closure WHERE ancestor=? AND descendant=?",
      )
      .get(a.id, c.id).depth,
    2,
  );
...
test("move: reparenting rebuilds the subtree closure; cycles rejected", () => {
  ...
  assert.throws(
    () => updateCollection(db, b.id, { parentId: c.id }),
    /own subtree/,
  );
  assert.throws(
    () => updateCollection(db, b.id, { parentId: b.id }),
    /own parent/,
  );
  db.close();
});
...
test("delete cascades the subtree (files survive, memberships drop)", () => {
  ...
  assert.equal(
    db.prepare("SELECT COUNT(*) c FROM collection_closure").get().c,
    0,
  );
  assert.equal(
    db.prepare("SELECT COUNT(*) c FROM file_collections").get().c,
    0,
    "membership gone",
  );
  assert.ok(
    db.prepare("SELECT 1 FROM files WHERE id = ?").get(x.id),
    "file survives",
  );
  db.close();
});
...
test("listCollections returns descendant-inclusive file counts", () => {
  ...
  const byId = Object.fromEntries(
    listCollections(db).map((c) => [c.id, c.fileCount]),
  );
...
test("visibility: isFilePublic true via direct or ancestor public; validated", () => {
  ...
  assert.equal(
    updateCollection(db, a.id, { visibility: "public" }).visibility,
    "public",
  );
  assert.equal(isFilePublic(db, inB.id), true);
  assert.equal(
    isFilePublic(db, inOther.id),
    false,
    "unrelated private collection stays private",
  );
  ...
  assert.throws(
    () => updateCollection(db, a.id, { visibility: "bogus" }),
    /visibility/,
  );
  db.close();
});
...
test("bulk membership: add/remove many files atomically", () => {
  ...
  assert.throws(
    () => addFilesToCollection(db, 9999, ids),
    /Collection not found/,
  );
  // ids[0] is currently NOT a member; batching it with a bad id must roll back its add.
  assert.throws(
    () => addFilesToCollection(db, a.id, [ids[0], 9999]),
    /File not found/,
  );
  assert.deepEqual(
    getFileCollectionIds(db, ids[0]),
    [],
    "no partial write from aborted batch",
  );
  db.close();
});
