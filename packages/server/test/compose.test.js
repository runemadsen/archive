...
test("parseQueryString extracts facet commands (: or =) and leaves the rest as text", () => {
  assert.deepEqual(parseQueryString("ext:jpg"), {
    text: "",
    filters: { ext: ["jpg"] },
  });
...
test("resolveState returns filters + normalized controls; reserved params are not facets", () => {
  const s = resolveState(
    new URLSearchParams(
      "q=trip&ext=jpg&sort=name&direction=asc&page=2&perPage=25",
    ),
  );
...
test("composeQuery renders canonical text + facet clauses", () => {
  assert.equal(
    composeQuery("mountains", { ext: ["jpg", "png"] }),
    "mountains ext=jpg,png",
  );
