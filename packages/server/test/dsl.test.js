...
import {
  ...
  compileQuery,
  QueryError,
} from "../src/lib/search/dsl.js";

test("tokenize keeps quoted segments (incl. spaces) intact", () => {
  assert.deepEqual(tokenize('a b "c d" key:"x y"'), [
    "a",
    "b",
    '"c d"',
    'key:"x y"',
...
test("comma value lists parse to multiple values and compile to OR", () => {
  ...
  assert.deepEqual(
    p.clauses[0].values.map((v) => v.text),
    ["jpg", "png"],
  );
  ...
  assert.deepEqual(
    q.clauses[0].values.map((v) => v.text),
    ["a,b"],
  );
});
...
test("compileQuery emits EXISTS for clauses and MATCH for text", () => {
  const { conditions, params } = compileQuery(
    parseQuery("mountain width>1920"),
  );
...
test("a free-text term matches FTS body OR a filename substring", () => {
  ...
  assert.ok(
    params.includes("%DSC%"),
    "binds a substring LIKE pattern for the filename",
  );
...
test("negation and != both produce NOT EXISTS", () => {
  assert.ok(
    compileQuery(parseQuery("-type:pdf")).conditions[0].startsWith(
      "NOT EXISTS",
    ),
  );
  assert.ok(
  ...
  assert.ok(
    compileQuery(parseQuery("-type!=pdf")).conditions[0].startsWith("EXISTS"),
  );
});
