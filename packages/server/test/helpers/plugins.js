...
const FIXTURE_ASSETS = fileURLToPath(
  new URL("../fixtures/assets/", import.meta.url),
);
...
export function fakeRegistry() {
  const text = {
    ...
    matches: (mime, filename) =>
      /^text\//.test(mime || "") || /\.(txt|md)$/i.test(filename || ""),
    async extract({ loadBuffer }) {
      ...
      return {
        metadata: [
          ...
          {
            key: "word_count",
            value: body.split(/\s+/).filter(Boolean).length,
            type: "number",
          },
          {
            key: "line_count",
            value: body === "" ? 0 : body.split("\n").length,
            type: "number",
          },
        ],
        fulltext: body,
    ...
    serving: {
      ...
      async serve({ segments, ext }, api) {
        ...
        return api.rendition({ spec }, ext, `image/${encoder}`, async () =>
          Buffer.from(
            `RENDITION:${encoder}:${spec.width ?? ""}x${spec.height ?? ""}`,
          ),
        );
      },
    },
  ...
  const video = {
    ...
    preview: (file, h) =>
      `<video controls data-hls="${h.url.serve("master.m3u8")}"></video>`,
    ...
    serving: {
      ...
      async pregenerate(_opts, api) {
        await api.buildBundle(async (outDir) => {
          ...
          await fsp.writeFile(
            path.join(outDir, "0", "index.m3u8"),
            "#EXTM3U\nseg_000.ts\n",
          );
          await fsp.writeFile(
            path.join(outDir, "0", "seg_000.ts"),
            Buffer.from("SEGMENT-BYTES"),
          );
        });
        return "hls";
      },
