...
test("upload requires authentication", async () => {
  ...
  try {
    const res = await app.upload("/api/files", {
      filename: "x.txt",
      body: "hi",
    });
...
test("re-uploading the same name with different content makes a new file", async () => {
  ...
  try {
    await login(app);
    const v1 = (
      await app.upload("/api/files", { filename: "doc.md", body: "v1" })
    ).json.file;
    const v2 = (
      await app.upload("/api/files", { filename: "doc.md", body: "v2 content" })
    ).json.file;
    ...
    assert.equal(
      (await app.get(`/api/files/${v2.id}/download`)).text,
      "v2 content",
    );
  } finally {
    await app.close();
...
test("same content under different names dedups the blob but makes distinct files", async () => {
  ...
  try {
    ...
    const a = (
      await app.upload("/api/files", { filename: "one.bin", body: "DUP" })
    ).json.file;
    const b = (
      await app.upload("/api/files", { filename: "two.bin", body: "DUP" })
    ).json.file;
...
test("soft-deleted file is gone from list and get (single-id array)", async () => {
  ...
  try {
    await login(app);
    const a = (await app.upload("/api/files", { filename: "z.txt", body: "z" }))
      .json.file;
    assert.equal(
      (await app.del("/api/files", { body: { fileIds: [a.id] } })).status,
      200,
    );
...
test("bulk delete: many files in one request", async () => {
  ...
  try {
    ...
    for (const name of ["a.txt", "b.txt", "c.txt"]) {
      ids.push(
        (await app.upload("/api/files", { filename: name, body: name })).json
          .file.id,
      );
    }
    assert.equal((await app.get("/api/files")).json.total, 3);

    const del = await app.del("/api/files", {
    ...
    assert.equal(
      (await app.del("/api/files", { body: { fileIds: [] } })).status,
      400,
    );
  } finally {
    await app.close();
