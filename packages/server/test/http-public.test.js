...
async function setup(app) {
  ...
  const parent = (await app.post("/api/collections", { name: "Pub" })).json
    .collection;
  const child = (
    await app.post("/api/collections", { name: "Child", parentId: parent.id })
  ).json.collection;
...
test("files in a public collection (or its subtree) are served; private are 404", async () => {
  ...
  try {
    ...
    await app.req("PATCH", `/api/collections/${parent.id}`, {
    ...
    const { txt } = {
      txt: (await app.get("/api/files")).json.items.find(
        (i) => i.original_filename === "n.txt",
      ),
    };
...
test("transform: content-type from extension, revalidated cache, 304, clamp, bad ext", async () => {
  ...
  try {
    ...
    // Revalidation → 304.
    const nm = await pub(app, `/i/${img.id}/w=400.webp`, {
...
test("transform on a non-servable public file → 404", async () => {
  ...
  try {
    ...
    await app.post(`/api/collections/${parent.id}/files`, {
      fileIds: [txt.id],
    });
    await app.req("PATCH", `/api/collections/${parent.id}`, {
      body: { visibility: "public" },
