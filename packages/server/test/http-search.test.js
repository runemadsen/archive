...
test("filename is searchable immediately, before extraction runs", async () => {
  ...
  try {
    ...
    assert.equal(
      (await app.get("/api/search?q=" + encodeURIComponent("type:image"))).json
        .total,
      1,
    );
  } finally {
...
test("search endpoint paginates and sorts with meta", async () => {
  ...
  try {
    ...
    const res = await app.get(
      "/api/search?sort=name&direction=asc&page=2&perPage=1",
    );
...
test("search endpoint filters uploaded-and-extracted files", async () => {
  ...
  try {
    ...
    const byType = await app.get(
      "/api/search?q=" + encodeURIComponent("type:text"),
    );
    assert.equal(byType.json.total, 2);

    // bad query -> 400
    const bad = await app.get(
      "/api/search?q=" + encodeURIComponent("type>image"),
    );
