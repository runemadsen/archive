...
test("download supports Range (206) and advertises Accept-Ranges", async () => {
  ...
  try {
    ...
    const part = await app.get(`/api/files/${id}/download`, {
      headers: { range: "bytes=0-4" },
    });
    assert.equal(part.status, 206);
    assert.equal(
      part.res.headers.get("content-range"),
      `bytes 0-4/${BODY.length}`,
    );
    ...
    const tail = await app.get(`/api/files/${id}/download`, {
      headers: { range: "bytes=18-" },
    });
    assert.equal(tail.status, 206);
    assert.equal(
      tail.res.headers.get("content-range"),
      `bytes 18-${BODY.length - 1}/${BODY.length}`,
    );
    assert.equal(tail.text, "TEST");

    // Unsatisfiable → 416.
    const bad = await app.get(`/api/files/${id}/download`, {
...
test("public /i/:id supports Range", async () => {
  ...
  try {
    ...
    const col = (await app.post("/api/collections", { name: "Pub" })).json
      .collection;
    await app.post(`/api/collections/${col.id}/files`, { fileIds: [id] });
    await app.req("PATCH", `/api/collections/${col.id}`, {
      body: { visibility: "public" },
    });

    const res = await fetch(`${app.base}/i/${id}`, {
