...
test("specSig is stable and distinguishes key + ext", () => {
  assert.equal(
    specSig({ width: 800 }, "webp"),
    specSig({ width: 800 }, "webp"),
  );
  assert.notEqual(
    specSig({ width: 800 }, "webp"),
    specSig({ width: 800 }, "jpg"),
  );
  assert.notEqual(
    specSig({ width: 800 }, "webp"),
    specSig({ width: 400 }, "webp"),
  );
});
...
test("servingFor picks a matching plugin that serves the extension", () => {
  const img = {
    id: "image",
    matches: (m) => /^image\//.test(m || ""),
    extract() {},
    serving: { formats: ["webp", "jpg"] },
  };
  const vid = {
    id: "video",
    matches: (m) => /^video\//.test(m || ""),
    extract() {},
    serving: { formats: ["m3u8", "ts"] },
  };
...
test("api.rendition generates once, then serves from the variant cache", async () => {
  ...
  try {
    const api = makeServingApi(
      ctx,
      { contentHash: hash, mimeType: "image/png", filename: "x.png" },
      plugin,
    );
    let runs = 0;
    const produce = async () => {
...
test("api.rendition returns null when the producer yields nothing (→ core 404/415)", async () => {
  ...
  try {
    ...
    assert.equal(
      await api.rendition({}, "webp", "image/webp", async () => null),
      null,
    );
  } finally {
    await cleanup();
...
test("api.buildBundle publishes atomically + idempotently; api.member locates or omits", async () => {
  ...
  try {
    const api = makeServingApi(ctx, { contentHash: hash }, plugin);
    assert.equal(
      api.member("master.m3u8", "application/vnd.apple.mpegurl"),
      null,
    ); // not built yet
    ...
    assert.ok(
      d && d.contentType === "application/vnd.apple.mpegurl" && d.size === 1,
    );
