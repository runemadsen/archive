...
test("worker pre-generates the HLS bundle and records stream_type", async () => {
  ...
  try {
    ...
    const row = app.db
      .prepare("SELECT stream_type, thumbnail_type FROM files WHERE id = ?")
      .get(id);
    ...
    const item = (await app.get("/api/files")).json.items.find(
      (i) => i.id === id,
    );
...
test("HTTP: HLS manifest + nested segment served by extension dispatch (no /hls/)", async () => {
  ...
  try {
    ...
    assert.equal(
      master.res.headers.get("content-type"),
    ...
    assert.equal(
      (await app.get(`/api/files/${id}/%2e%2e%2fseg_000.ts`)).status,
      404,
    );
...
test("plugin assets are served, with traversal rejected", async () => {
  ...
  try {
    ...
    assert.equal(
      (await app.get("/plugin-assets/video/%2e%2e%2f%2e%2e%2fpackage.json"))
        .status,
      404,
    );
