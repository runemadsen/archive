...
test("extraction pre-generates the thumbnail and records thumbnail_type", async () => {
  ...
  try {
    ...
    assert.equal(
      s.db.prepare("SELECT thumbnail_type FROM files WHERE id = ?").get(fileId)
        .thumbnail_type,
      "image/webp",
    );
  } finally {
...
test("no derivedStore => no thumbnail generated", async () => {
  ...
  try {
    ...
    const res = await runExtraction(
      s.db,
      { blobStore: s.blobStore, registry: fakeRegistry() },
      fileId,
    );
...
test("HTTP: thumbnail served for images, 404 for text; list flags thumbnail_type", async () => {
  const app = await startTestApp({
    onFileCreated: (id) => enqueueExtraction(app.db, id),
  });
  try {
    ...
    const img = await app.upload("/api/files", {
      filename: "p.png",
      contentType: "image/png",
      body: "imgbytes",
    });
    const txt = await app.upload("/api/files", {
      filename: "n.txt",
    ...
    assert.equal(
      (await app.get(`/api/files/${txt.json.file.id}/thumbnail`)).status,
      404,
    );

    const list = await app.get("/api/files");
    const byName = Object.fromEntries(
      list.json.items.map((i) => [i.original_filename, i.thumbnail_type]),
    );
...
test("HTTP (production): thumbnails are immutable and support If-None-Match", async () => {
  ...
  try {
    ...
    assert.ok(etag);
    const revalidated = await app.get(`/api/files/${id}/thumbnail`, {
