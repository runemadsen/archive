...
test("findDuplicateFile matches on filename AND content hash only", () => {
  ...
  assert.ok(
    findDuplicateFile(db, { filename: "photo.jpg", hash: "H1" }),
    "exact match",
  );
  assert.equal(
    findDuplicateFile(db, { filename: "photo.jpg", hash: "H2" }),
    null,
    "same name, different content",
  );
  assert.equal(
    findDuplicateFile(db, { filename: "other.jpg", hash: "H1" }),
    null,
...
test("HTTP: re-uploading the same file+name is skipped; variations still import", async () => {
  ...
  try {
    ...
    const first = await app.upload("/api/files", {
      filename: "a.txt",
    ...
    const dup = await app.upload("/api/files", {
      filename: "a.txt",
      contentType: "text/plain",
      body: "hello",
    });
...
test("HTTP: uploading 10 where 5 exist imports only the remaining 5", async () => {
  ...
  try {
    ...
    let skipped = 0;
    for (let i = 0; i < 10; i++) {
      const r = await app.upload("/api/files", {
        filename: `f${i}.txt`,
        contentType: "text/plain",
        body: `content ${i}`,
      });
