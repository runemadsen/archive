...
test("extraction records core metadata for any file", async () => {
  ...
  try {
    ...
    await runExtraction(
      s.db,
      { blobStore: s.blobStore, registry: new PluginRegistry() },
      vid,
    );
...
test("text plugin adds counts and full text is searchable", async () => {
  ...
  try {
    const vid = await s.seedFile({
      ...
      buffer: Buffer.from("mountain sky\ntree river"),
    });
    await runExtraction(
      s.db,
...
test("multiple plugins merge; a failing plugin is isolated", async () => {
  ...
  try {
    ...
    const tagger = {
      ...
      extract: async () => ({
        metadata: [
          { key: "label", value: "mountain", type: "text" },
          { key: "label", value: "sky", type: "text" },
        ],
      }),
    };
    ...
    const result = await runExtraction(
      s.db,
      { blobStore: s.blobStore, registry },
      vid,
    );
    ...
    assert.deepEqual(
      result.pluginErrors.map((e) => e.plugin),
      ["boom"],
    );
  } finally {
    await s.cleanup();
...
test("re-running extraction is idempotent (no duplicate rows)", async () => {
  ...
  try {
    ...
    assert.equal(
      s.db
        .prepare("SELECT COUNT(*) c FROM metadata_fts WHERE file_id = ?")
        .get(vid).c,
      1,
    );
  } finally {
...
test("queue: enqueue dedups and runPending drains + sets status", async () => {
  ...
  try {
    ...
    const processed = await runPending(s.db, {
    ...
    const status = s.db
      .prepare("SELECT extraction_status FROM files WHERE id = ?")
      .get(vid).extraction_status;
